...
class HandDetection:
    ...
    def find_contours(self, mask):
        # TODO : Complete this function to return contours from the mask (Apply contour approximation also)
        contours = []
        contours,_ = cv2.findContours(mask,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_NONE)
        max_cnts = sorted(contours,key=lambda x: cv2.contourArea(x),reverse=True)
        approx_contours = []
        for contour in max_cnts:
    ...
    def draw_contours(self,contours,frame):
        # TODO : Complete this function to draw contours as well as centroids on frame
        for contour in contours:
            cv2.drawContours(frame,[contour],-1,(0,255,0),2)
            x,y = self.centroid(contour)
            frame = cv2.circle(frame,(x,y),4,(255,0,0),-1)  
            return     frame
...
while True:
    ...
    contours = hd.find_contours(mask)

    frame = hd.draw_contours(contours,frame)
